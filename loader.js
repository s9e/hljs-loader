(function (window, document)
{
	...
	*/
	...
	{
		let aliases = '1c abnf accesslog actionscript,as ada angelscript,asc applescript,osascript arcade arduino armasm,arm asciidoc,adoc aspectj autohotkey,ahk autoit avrasm awk axapta basic bnf brainfuck,bf cal capnproto,capnp ceylon clean,dcl,icl clojure,clj clojure-repl cmake coq cos,cls crmsh,crm,pcmk crystal,cr csp d dart delphi,dfm,dpr,freepascal,lazarus,lfm,lpr,pas,pascal django,jinja dns,bind,zone dockerfile,docker dos,bat,cmd dsconfig dts dust,dst ebnf elixir elm erb erlang,erl erlang-repl excel,xls,xlsx fix flix fortran,f90,f95 fsharp,fs gams,gms gauss,gss gcode,nc gherkin,feature glsl gml go,golang golo gradle groovy haml handlebars,hbs haskell,hs haxe,hx hsp htmlbars hy,hylang inform7,i7 irpf90 isbl jboss-cli,wildfly-cli julia julia-repl,jldoctest kotlin,kt lasso,lassoscript,ls ldif leaf less lisp livecodeserver livescript,ls llvm lsl lua mathematica,mma,wl matlab maxima mel mercury,m,moo mipsasm,mips mizar mojolicious monkey moonscript,moon n1ql nimrod,nim nix,nixos nsis ocaml,ml openscad,scad oxygene parser3 pf pgsql,postgres,postgresql plaintext pony powershell,ps,ps1 processing profile prolog protobuf puppet,pp purebasic,pb,pbi q,k,kdb qml,qt r reasonml,re rib roboconf,graph,instances routeros,mikrotik rsl ruleslanguage rust,rs sas scala scheme scilab,sci scss smali smalltalk,st sml,ml sqf stan stata,ado,do step21,p21,step,stp stylus,styl subunit swift taggerscript tap tcl,tk tex thrift tp twig,craftcms typescript,ts vala vbnet,vb vbscript,vbs vbscript-html verilog,sv,svh,v vhdl vim x86asm xl,tao xquery,xpath,xq zephir,zep'.split(' '),
			i       = aliases.length,
			map     = {};
		while (--i >= 0)
		{
	...
	*/
	...
	let config        = document.currentScript ? document.currentScript.dataset : {},
		...
		url           = config['hljsUrl']   || 'https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@9.15.10/build/';
