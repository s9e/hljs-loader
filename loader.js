(function (window, document)
{
	...
	*/
	...
	{
		let aliases = '1c abnf accesslog actionscript,as ada angelscript,asc apache,apacheconf applescript,osascript arcade arduino armasm,arm asciidoc,adoc aspectj autohotkey,ahk autoit avrasm awk axapta,x++ basic bnf brainfuck,bf cal capnproto,capnp ceylon clean,dcl,icl clojure,clj clojure-repl cmake coffeescript,coffee,cson,iced coq cos,cls crmsh,crm,pcmk crystal,cr csp d dart delphi,dfm,dpr,pas,pascal django,jinja dns,bind,zone dockerfile,docker dos,bat,cmd dsconfig dts dust,dst ebnf elixir elm erb erlang,erl erlang-repl excel,xls,xlsx fix flix fortran,f90,f95 fsharp,fs gams,gms gauss,gss gcode,nc gherkin,feature glsl gml golo gradle groovy haml handlebars,hbs,htmlbars haskell,hs haxe,hx hsp http,https hy,hylang inform7,i7 irpf90 isbl jboss-cli,wildfly-cli julia julia-repl,jldoctest kotlin,kt,kts lasso,lassoscript,ls latex,tex ldif leaf lisp livecodeserver livescript,ls llvm lsl mathematica,mma,wl matlab maxima mel mercury,m,moo mipsasm,mips mizar mojolicious monkey moonscript,moon n1ql nestedtext,nt nginx,nginxconf nim nix,nixos node-repl nsis ocaml,ml openscad,scad oxygene parser3 pf pgsql,postgres,postgresql pony powershell,ps,ps1 processing,pde profile prolog properties protobuf puppet,pp purebasic,pb,pbi q,k,kdb qml,qt reasonml,re rib roboconf,graph,instances routeros,mikrotik rsl ruleslanguage sas scala scheme scilab,sci smali smalltalk,st sml,ml sqf sql stan,stanfuncs stata,ado,do step21,p21,step,stp stylus,styl subunit taggerscript tap tcl,tk thrift tp twig,craftcms vala vbscript,vbs vbscript-html verilog,sv,svh,v vhdl vim wasm wren x86asm xl,tao xquery,xpath,xq zephir,zep'.split(' '),
			i       = aliases.length,
			map     = {};
		while (--i >= 0)
		{
	...
	*/
	...
	let config        = document.currentScript.dataset,
		...
		url           = config['hljsUrl']   || 'https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11.0.0/build/';
